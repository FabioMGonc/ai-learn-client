...
const Home = () => {
  return (
    <main className="page space-y-20 bg-white/75 animate-in fade-in duration-500 sm:mt-14">
      <section className="grid gap-10 lg:grid-cols-12 lg:items-start">
        <div className="space-y-5 lg:col-span-7">
          <Badge variant="secondary" className="h-8 gap-1.5 px-3 text-xs font-medium">
          ...
          <h1 className="text-4xl  text-zinc-800 font-bold tracking-tight sm:text-5xl lg:[3.5rem] lg:leading-[1.05]">
            ...
            <span className="bg-linear-to-r from-[#10A0F0] to-[#0040A0] bg-clip-text text-transparent">Tutores de Voz com IA</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/tutors">
              <Button className="rounded-full p-5 sm:p-6 text-base font-semibold shadow-lg shadow-primary/20">
                Conheça Tutores<ArrowRight className="ml-2 size-4 transition-transform group-hover:translate-x-1" />
              </Button>
            </Link>

            <Link href="/tutors/new">
              <Button className="bg-popover hover:bg-popover/90 rounded-full p-5 sm:p-6 text-base font-semibold shadow-lg shadow-primary/20">
                Crie o seu
              </Button>
            </Link>
          </div>
              ...
              {stats.map(({ icon: Icon, value, label, desc }) => {
                return (
                  <div key={label} className="stat-card shadow-lg shadow-primary/20">
                    <div className="mb-3 items-center justify-center rounded-lg">
